...
class Character {
    ...
    botMoveHorz(xDiff){
       if (xDiff < 0) {
           return this.validMove(this.pos, [this.pos[0] + 24, this.pos[1]]) ? "ArrowRight" : false;
       }
        return this.validMove(this.pos, [this.pos[0] - 24, this.pos[1]]) ? "ArrowLeft" : false;
    }

    botMoveVert(yDiff){
       if (yDiff < 0) {
           return this.validMove(this.pos, [this.pos[0], this.pos[1] + 24]) ? "ArrowDown" : false;
       }
        return this.validMove(this.pos, [this.pos[0], this.pos[1] - 24]) ? "ArrowUp" : false;
    }

    move (direction) {
        if (direction === false) return;
        if (direction === "Right" || direction === "ArrowRight") {
            this.pos[0] = this.validMove(this.pos, [this.pos[0] + 24, this.pos[1]]) ? this.pos[0] + 24 : this.pos[0];
        }
        else if (direction === "Left" || direction === "ArrowLeft") {
            this.pos[0] = this.validMove(this.pos, [this.pos[0] - 24, this.pos[1]]) ? this.pos[0] - 24 : this.pos[0];
        }
        else if (direction === "Up" || direction === "ArrowUp") {
            this.pos[1] = this.validMove(this.pos, [this.pos[0], this.pos[1] - 24]) ? this.pos[1] - 24 : this.pos[1];
        }
